...
if not frozen_path.exists():
    raise FileNotFoundError(
        "Run notebooks/02_analysis.ipynb before rendering so the frozen "
        "Method 2 table can be verified."
    )
stored = pd.read_csv(frozen_path)
pd.testing.assert_frame_equal(
    results, stored, check_dtype=False, check_exact=False,
...
The displayed results are independently reproduced from the immutable OECD
input and must match the tracked Method 2 table before rendering continues.
...
if not trajectory_figure.exists():
    raise FileNotFoundError(
        "Run notebooks/04_final_visuals.ipynb before rendering the report."
    )
```

![Australia and same-period supplied-country distributions. The shaded band is
the interquartile range; social observations are independent three-year pooled
windows.](../reports/figures/material_social_trajectories.png){#fig-trajectories width=100%}
