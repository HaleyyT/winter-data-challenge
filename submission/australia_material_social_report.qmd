...
eligible supplied-country comparator on both social outcomes.

> **Australia became materially better resourced, while perceived social
> support and emotional well-being worsened relative to eligible
> supplied-country comparators.**
...
However, Australia's improvements in income and employment did not consistently
exceed the median across eligible supplied-country comparators. Only the
available S80/S20 income ratio slightly changed between 2010 and 2024.
...
```{python}
#| label: method2-common-endpoints
AUSTRALIA = "AUS"
OUTCOMES = {
...
::: {.primary-scorecard}
```{python}
#| label: tbl-method2-scorecard
display(
    scorecard.round(2).style
    .set_uuid("method2-scorecard")
    .hide(axis="index")
    .format({
        "AU change": "{:,.2f}",
        "Median change": "{:,.2f}",
...
### Method 5: Exploratory material-social rank association

Method 5 asks whether countries with stronger material improvement also tended
to show more favourable social change. For each of the four pre-specified
material-social pairs, Australia is excluded from estimating Spearman's rho,
paired country bootstraps, and permutation p-values. Holm adjustment is applied
across the four tests. This remains ecological and exploratory; it does not
identify a causal pathway.
...
```{python}
#| label: check-method1-trajectory-output
#| output: false
...
::: {.trajectory-figure}
![Australia's material and social trajectories relative to supplied-country
distributions. Lines show Australia and the same-period median among supplied
countries excluding Australia; shaded bands show the interquartile range.
Income is measured in PPP-converted USD per person. Social indicators are
independent three-year pooled estimates, with lower values indicating more
favourable outcomes. Connecting lines are visual guides rather than annual
observations, and country coverage varies by outcome and
period.](../reports/figures/material_social_trajectories.png){#fig-trajectories width=100%}
:::

### Robustness: endpoint and full-trajectory directions agree

```{python}
#| label: load-method4-trend-output
...
::: {.trend-agreement}
```{python}
#| label: tbl-method4-trend-agreement
display(
    trend_table.style
    .set_uuid("method4-trend-agreement")
    .hide(axis="index")
...
### Exploratory material-social associations are strongest for negative affect

```{python}
#| label: tbl-method5-spearman
spearman_path = ROOT / "reports" / "tables" / "material_social_spearman_results.csv"
if not spearman_path.exists():
    raise FileNotFoundError("Run notebooks/method5_spearman_association.ipynb before rendering the report.")
...
assert spearman["pair_id"].tolist() == expected_pairs
assert spearman["comparator_country_count"].tolist() == [31, 31, 43, 43]
assert spearman["fixed_common_panel_count"].tolist() == [31, 31, 31, 31]
assert spearman["spearman_rho"].between(-1, 1).all()
assert spearman["bootstrap_ci_lower"].between(-1, 1).all()
assert spearman["bootstrap_ci_upper"].between(-1, 1).all()
assert (spearman["bootstrap_ci_lower"] <= spearman["bootstrap_ci_upper"]).all()
...
spearman_table = pd.DataFrame({
    ...
    "n": spearman["comparator_country_count"].astype(int),
    "Spearman rho": spearman["spearman_rho"],
    "95% bootstrap CI": spearman.apply(
        lambda row: f"[{row['bootstrap_ci_lower']:.3f}, {row['bootstrap_ci_upper']:.3f}]",
        axis=1,
    ),
...
display(
    spearman_table.style
    .set_uuid("method5-spearman")
    .hide(axis="index")
    .format({"Spearman rho": "{:.3f}", "Holm p": "{:.4f}"})
    .set_caption("Exploratory Method 5 material-social rank associations")
)
```

Stronger material improvement generally ranked alongside more favourable social
change, particularly for negative affect. The income-negative-affect and
employment-negative-affect associations remain supported after Holm adjustment.
The social-support associations are positive, but their Holm-adjusted p-values
are 0.0819, so they are statistically less certain. This exploratory evidence
therefore does not support claiming that all four associations are significant.
...
supplied-country comparators. Within the supplied data, Australia therefore
became materially better resourced while its measured social and emotional
outcomes deteriorated comparatively. Stronger material improvement generally
ranked alongside more favourable social change, particularly for negative
affect. Australia nevertheless experienced material improvement alongside
adverse social change. This is exploratory ecological evidence and does not
identify a causal explanation.

::: {.trend-agreement}
| Claim | Classification | Basis |
|---|---|---|
| Australia improved materially while social support and negative affect deteriorated | Robust | Endpoint changes and Theil--Sen slopes agree for all four primary outcomes |
| Australia's social deterioration was worse than the typical eligible supplied-country comparator | Robust | Method 2 endpoint ranks place social support near the 9th favourable percentile and negative affect near the 20th |
| Australia materially outperformed the broad supplied-country reference | Unsupported | Income and employment improved domestically but were below their eligible comparator medians |
| Material improvement across countries always predicts social improvement | Comparator-sensitive | Method 5 associations are positive, but only the negative-affect pairs remain supported after Holm adjustment |
:::
...
### Method 5 diagnostic figure

The material-improved/social-worsened quadrant is not unique to Australia. It
contains about 58%, 39%, 49% and 40% of the respective eligible comparator
samples for the four Method 5 pairs. Australia is notable mainly for the
severity and low comparative ranking of its social deterioration, especially
social support.

![Exploratory cross-country associations between material and social change.
Positive values indicate improvement. Australia is shown separately and is
excluded from each Spearman rho, bootstrap interval and permutation
p-value.](../reports/figures/material_social_spearman_associations.png){#fig-method5-spearman width=100%}
...
The HTML output is currently tracked for final submission review. A successful
render verifies the raw input, audit, report calculations, chart generation,
and repository tests in one run.
